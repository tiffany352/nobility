...
impl<'a> IntArray<'a> {
  ...
  pub fn get(&self, index: usize) -> Option<i32> {
    if index < self.len() {
      (&self.0[index*4..index*4+4]).read_i32::<BigEndian>().ok()
    } else {
      None
    }
  }
...
fn decode_value<'a>(tag: TagType, reader: &mut Reader<'a>) -> Result<Tag<'a>, Error> {
  match tag {
    ...
    TagType::Short => reader.advance(2).and_then(
      |mut v| Ok(Tag::Short(try!(v.read_i16::<BigEndian>().map_err(Error::ByteOrderError))))),
    TagType::Int => reader.advance(4).and_then(
      |mut v| Ok(Tag::Int(try!(v.read_i32::<BigEndian>().map_err(Error::ByteOrderError))))),
    TagType::Long => reader.advance(8).and_then(
      |mut v| Ok(Tag::Long(try!(v.read_i64::<BigEndian>().map_err(Error::ByteOrderError))))),
    TagType::Float => reader.advance(4).and_then(
      |mut v| Ok(Tag::Float(try!(v.read_f32::<BigEndian>().map_err(Error::ByteOrderError))))),
    TagType::Double => reader.advance(8).and_then(
      |mut v| Ok(Tag::Double(try!(v.read_f64::<BigEndian>().map_err(Error::ByteOrderError))))),
    TagType::ByteArray => {
      let len = {
    ...
    TagType::String => {
      ...
      Ok(Tag::String(try!(reader.advance(len as usize).and_then(|v| from_utf8(v).map_err(Error::Utf8Error)))))
    }
    TagType::List => {
      let mut list = vec![];
      let tag = try!(reader.advance(1).and_then(
        |v| TagType::from_raw(v[0]).ok_or(Error::UnknownTag)));
      let len = try!(reader.advance(4).and_then(|mut v| v.read_u32::<BigEndian>().map_err(Error::ByteOrderError)));
    ...
    TagType::IntArray => {
      ...
      Ok(Tag::IntArray(IntArray(try!(reader.advance(len as usize * 4)))))
    }
  }
}
...
fn decode_full<'a>(reader: &mut Reader<'a>) -> Result<Option<(&'a str, Tag<'a>)>, Error> {
  ...
  let name_len = try!(reader.advance(2).and_then(
    |mut v| v.read_u16::<BigEndian>().map_err(Error::ByteOrderError)));
  let name = try!(reader.advance(name_len as usize).and_then(|v| from_utf8(v).map_err(Error::Utf8Error)));
